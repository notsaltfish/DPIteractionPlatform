        ...
        <div class="modal-dialog" style="width:300px;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>
                        <label  class="control-label">病种名称</label>
                ...
                <script>
                    ...
                    $("#add").click(function(){
                        $("#idDiv").hide();
                        $("#myModalLabel").text("添加病种");
                        $("#modalSub").attr("onclick","add()");
                        $('#myModal').modal('show');
                        var name = $("input[name='diseaseName']").val("")
                    });
                    ...
                    var id = $("input[name='id']").val()
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            dataType: "json",
                            data: {type:"disease",id:id,name:name},
                            success: function (data) {
                                query();
                            },
                            error:function(){
                    ...
                    function modify(obj){
                       var id = $(obj).parent("td").parent("tr").find("td:eq(0)").text();
                        var name = $(obj).parent("td").parent("tr").find("td:eq(2)").text();

                        $("input[name='id']").val(id);
                        $("input[name='diseaseName']").val(name);
                    }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var item of data.datas) {
                        ...
                        $template.children('td').eq(1).text(item.icdCode==null?"无ICD编码":item.icdCode);
                        $template.children('td').eq(2).text(item.name=='null'?"":item.name);
